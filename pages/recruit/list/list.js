...
Page({
    ...
    data: {
        ...
        isRefreshing:false,
        isLoadMore:false,
        imgHeight:wx.getSystemInfoSync().windowWidth*0.5
    },
    ...
    fetchData: () => {
        ...
        Util.request('/recruit/batchget_recruit', {
            catid,
            offset,
            count:10,
        }).then(response => {
            response.data.items.forEach((item) => {
                item.formatted_time = moment(new Date(item.created_at * 1000)).format('YYYY-MM-DD');
            });
